...
**Augmentation -- a real controlled result, and a correction along the way.** Random 90-degree rotation/flip is physically valid for a wafer map (no inherent "up"). An early single-run comparison suggested it *hurt* this model -- but that comparison turned out to be invalid: `train_and_save` wasn't fully seeded (torch's own RNG and the augmentation's own `np.random` calls were uncontrolled), so two "identical" runs could land anywhere in a 70.8%-74.4% test-macro-F1 band from random-init noise alone, nowhere near enough signal to trust a single-run A/B. After adding `torch.manual_seed`/`np.random.seed` and re-running a real same-seed controlled comparison (`scripts/train_wafer_cnn.py --ab-test`), the answer reversed: `augment=True` scored 69.8% test macro-F1 / 93.2% accuracy vs. `augment=False`'s 69.0% / 88.7% at the identical seed -- a real, if modest, improvement. `augment=True` is the default for that reason, not the original (wrong) one.
