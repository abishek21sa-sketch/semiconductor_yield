...
- **SMT2020** (`data/smt2020_lvhm/` and `data/smt2020_hvlm/`, both bundled): a published academic semiconductor-fab simulation benchmark (Kopp, Hassoun, Kalir & Mönch, *IEEE TSM*, 2020), used in two of its published archetype configurations. Synthetic/benchmark routing and reliability data representing fab archetypes, not a real company's operational data. See each directory's `ATTRIBUTION.md`.
...
- `data/wm811k/LSWMD.pkl` and the trained wafer-CNN/baseline artifacts derived from it are gitignored too -- not secrets, just too large to commit (~2.1GB raw file). `data/wm811k/ATTRIBUTION.md` itself is still tracked.
